...
{
    $sql = "
    SELECT  users.username, users.Address, users.name, users.PhoneNumber, products.ProductName, Status.StatusID, products.Price, orders.quantity, orders.OrderID, orders.OrderDate, paymentmethod.PaymentMethodName, Status.StatusName
    FROM users
...
{
    $sql = "SELECT * FROM status order by StatusID asc limit 4";
