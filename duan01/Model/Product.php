...
{
    $sql = "SELECT * FROM products order by ProductID asc limit 8 ";
...
function renderTop()
{
    $sql = "SELECT * FROM products order by ProductID limit 7" ;
    $renderTopProduct = pdo_query($sql);
    return $renderTopProduct;
}
...
function renderProductRelated($ProductID)
{
    $sql = "SELECT * FROM imgproduct WHERE ProductID=" . $ProductID;
...
{
    $sql = "SELECT img_products.*, products.*
    FROM img_products
    JOIN products ON img_products.ProductID = products.ProductID
    WHERE img_products.ProductID = $ProductID
    ORDER BY ImgID DESC
    ";
